...
# Restore dataframe across reruns / page switches
if getattr(controller, "df", None) is None and "df" in st.session_state:
    controller.df = st.session_state.df
    controller.dataset_name = st.session_state.get("dataset_name", None)
...
if page == "Import Data":
    ...
    if uploaded is not None:
        try:
            ...
            # Persist to session_state so other pages can use it
            st.session_state.df = controller.df
            st.session_state.dataset_name = controller.dataset_name
    ...
    tab_clean, tab_filter, tab_sort, tab_group = st.tabs(
        ["Missing Values", "Filter", "Sort", "Group & Aggregate"]
    )
    ...
    with tab_clean:
        ...
        if strategy == "Fill missing (custom)":
            custom_val = st.text_input(
                "Custom fill value (applies to all columns where possible)",
                value="0",
            )
    ...
    chart_type = st.selectbox(
        "Chart type",
        ["Bar", "Line", "Scatter", "Histogram", "Correlation Heatmap"],
    )
    ...
    if chart_type in ["Bar", "Line", "Scatter"]:
        ...
        if chart_type == "Bar":
            y_candidates = numeric_columns(df)  # bar should aggregate numeric
        else:
            y_candidates = numeric_columns(df)

        if not y_candidates:
            st.info("No numeric columns available for this chart type.")
        else:
            y = st.selectbox("Y axis", y_candidates)

            color_opt = st.selectbox("Color (optional)", ["(none)"] + all_columns(df))
        ...
        if not nums:
            ...
            cols = st.multiselect("Columns (numeric)", nums, default=nums[:6])
    ...
    # Restore last figure if needed
    if getattr(controller, "last_figure", None) is None and "last_figure" in st.session_state:
        controller.last_figure = st.session_state.last_figure
    ...
    if controller.last_figure is None:
        ...
        # Always provide a fallback that works everywhere (HTML)
        st.caption("Fallback export (always works on Streamlit Cloud):")
        try:
            html = controller.last_figure.to_html(include_plotlyjs="cdn")
            st.download_button(
